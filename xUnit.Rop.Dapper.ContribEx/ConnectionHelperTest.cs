    ...
    public class ConnectionHelperTest
    {
        ...
        public void GetWhereTest()
        {
            using (var conn=GetOpenConnection())
            {
                ...
                conn.Insert(c2);
                conn.Insert(c3);
                var lst1 = conn.GetWhere<Car>("Name=@name", new { name = "Ford" });
                Assert.Equal(new[]{1,2},lst1.Select(c=>c.Id));
            }
        }
        [Fact]
        public void QueryIdValue()
        {
            using (var conn=GetOpenConnection())
            {
                ...
                conn.Insert(c2);
                conn.Insert(c3);
                var lst1 = conn.QueryIdValue<User,int>("Age","Name=@name", new { name = "Pepe" }).OrderBy(c=>c.id).Select(c=>((int)c.id,c.value));
        ...
        public void GetIdValues()
        {
            using (var conn=GetOpenConnection())
            {
                ...
                conn.Insert(c2);
                conn.Insert(c3);
                var lst1 = conn.GetIdValues<User,int>(new[]{ 1,2 },"Age").OrderBy(c=>c.id).Select(c=>((int)c.id,c.value));
                Assert.Equal(new(int,int)[]{(c1.Id,c1.Age),(c2.Id,c2.Age)},lst1);
            }
        }
